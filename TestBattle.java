...
public class TestBattle {
	...
	public TestBattle(BattleState [] teamA, BattleState [] teamB, boolean debugModeOn){ 
		teamBob = teamA;
		teamRob = teamB;
		isDebug = debugModeOn;
	...
	public void battleSim()
	{
		...
		if (robCount <= 0){
			//bob won
			if (isDebug) System.out.println("Team Bob wins the battle, with " + bobCount + " entities remaining.");
		}
		else if (bobCount <= 0){
			//rob won
			if (isDebug) System.out.println("Team Rob wins the battle, with " + robCount + " entities remaining.");
		} else
		{//a draw
	...
	private BattleState pickTarget(BattleState isTurn, int team){
		...
		//TODO change this to modify targeting priorities
			targ = rng.nextInt() % targetList.length;
